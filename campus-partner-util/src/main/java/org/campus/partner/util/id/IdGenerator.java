...
public class IdGenerator implements Generator {
    ...
    /**
     * 获取分布式全局唯一ID（WSID）. <br/>
     * 指定一个前缀的方式生成ID的字符串表示形式. eg:XXXX0000016ce3113ec3005056c000080001
     * 
     * @param prefix
     *            为ID指定一个前缀
     * @return 应用全局唯一ID(WSID)的字符串表示
     ...
     * @since 1.0.0
     */
    public String getWsid(String prefix) {
        return prefix == null ? getWsid() : prefix.concat(getWsid());
    }
    ...
    public byte[] getWsidBytes(String prefix) {
        return getWsid(prefix).getBytes();
    }
     ...
     */
    public String getId() {
        return getWsid(DEFAULT_ID_START.concat(DEFAULT_ID_SEPARATOR));
    }

    public static void main(String[] args) {
        IdGenerator wsidGenerator = new IdGenerator();
        System.out.println(wsidGenerator.getWsid());
    }
}